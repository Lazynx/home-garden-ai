...
class DiseaseService {
        ...
        messages: [
          {
            role: 'system',
            content: `
            Вы ботаник, специализирующийся на болезнях растений. Задайте уточняющий вопрос или выдай диагноз на русском и английском языках. В диагнозе распиши подробно проблему и чем она возможна была вызвана. Кроме того, если нужны удобрения - ВЫДАЙ ТОЧНЫЕ НАЗВАНИЯ УДОБРЕНИЙ для решения проблемы.  Ответ в формате JSON:
              [
                {
                  "question": "Ваш уточняющий вопрос на русском",
                  "questionEn": "Your clarifying question in English",
                  "diagnosis": "Диагноз на русском, если все вопросы заданы",
                  "diagnosisEn": "Diagnosis in English, if all questions are asked"
                }
              ]
            `
          },
  ...
  async diagnosePlantDisease(
    ...
    try {
      ...
      return {
        questions: questionData.map((item: any) => item.question),
        questionsEn: questionData.map((item: any) => item.questionEn),
        initialDiagnosis: questionData[0].diagnosis,
        initialDiagnosisEn: questionData[0].diagnosisEn,
        context: [
          {
            role: 'system',
            content: `
  ...
  async continueDiagnosis(
    ...
    try {
      ...
      return {
        ...
        questionsEn: questionData.map((item: any) => item.questionEn),
        finalDiagnosis: questionData[0].diagnosis,
        finalDiagnosisEn: questionData[0].diagnosisEn,
        context: [
          ...previousContext,
          {
            role: 'user',
